...
class CPG(object):
    ...
    def update_parameters(self, replay_buffer, batch_size=256):
        ...
        state, action, next_state, reward, mask, _, mu = replay_buffer.sample(batch_size)
        ...
        with torch.no_grad():
            ...
            target_Q = reward + mask * self.gamma * target_Q
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            with torch.no_grad():
                finite_diff = self.critic.Q1(state, current_action.detach() + mu * noise) - self.critic.Q1(state, current_action.detach())
                scale = finite_diff * noise / mu

            actor_loss = -(current_action * scale).mean()
